...
export const getAllGAPs = async (): Promise<GAP[]> => {
  try {
    ...
    // Obtener usuarios para llenar los nombres
    const allUsers = await getAllUsuarios();
    mapped.forEach(gap => {
      if (gap.pastorId) {
        const u = allUsers.find(x => x.id === gap.pastorId);
        if (u) gap.pastorNombre = `${u.nombre} ${u.apellidos}`;
      }
      if (gap.liderMentorId) {
        const u = allUsers.find(x => x.id === gap.liderMentorId);
        if (u) gap.liderMentorNombre = `${u.nombre} ${u.apellidos}`;
      }
      if (gap.liderGapId) {
        const u = allUsers.find(x => x.id === gap.liderGapId);
        if (u) gap.liderGapNombre = `${u.nombre} ${u.apellidos}`;
      }
      if (gap.timoteoId) {
        const u = allUsers.find(x => x.id === gap.timoteoId);
        if (u) gap.timoteoNombre = `${u.nombre} ${u.apellidos}`;
      }
    });
...
export const getGAPById = async (id: string): Promise<GAP | null> => {
  ...
  if (error || !data) return null;
  const gap = fromDbGAP(data as DbRecord);
...
function fromDbGAP(db: DbRecord): GAP {
  return {
    ...
    liderGapId: db.lider_gap_id as string, liderGapNombre: (db.lider_gap_nombre as string) || 'LÃ­der Desconocido',
    ...
    liderMentorId: db.lider_mentor_id as string, liderMentorNombre: (db.lider_mentor_nombre as string) || 'LÃ­der Mentor Desconocido',
...
function fromDbEvento(db: DbRecord): EventoCalendario {
  return {
    id: db.id as string, titulo: db.titulo as string, descripcion: db.descripcion as string,
    tipo: (db.tipo as any) || 'ReuniÃ³n', fecha: db.fecha as string, hora: db.hora as string | undefined,
